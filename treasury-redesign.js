...
(function(){
  ...
  function home(){ const s=GT.snapshot||{}; const adv=(s.personalAdvances||[]).map(a=>'<div class="treasury-row"><div><strong>'+h(a.player_id?pnameById(a.player_id):'Legacy sin beneficiaria')+'</strong><div class="muted">Original '+money(a.original_amount)+' - Recuperado '+money(a.recovered_amount)+'</div></div><span class="pill warn">'+money(a.pending_amount)+'</span></div>').join('')||'<div class="empty-state">Sin adelantos personales pendientes</div>'; document.getElementById('treasuryViewBody').innerHTML='<section class="treasury-panel"><div class="treasury-panel-title">Dinero personal pendiente de devolver</div>'+adv+'</section><section class="treasury-panel"><div class="treasury-panel-title">Pago DT</div><div class="treasury-dt-box"><div><strong>'+money(s.dtPending&&s.dtPending.amount)+'</strong><div class="muted">'+(s.dtPending&&s.dtPending.count||0)+' cuota(s)</div></div><button class="btn p" data-action="pay-dt">Pagar DT</button></div></section>'; }
  ...
  function activities(){ const s=GT.snapshot||{}; const legacy=s.mode==='legacy'?'<div class="treasury-note">Las tablas legacy se leen solo como referencia. Nuevas actividades requieren SQL/RPC.</div>':''; const rows=(s.activities||[]).map(a=>{ const all=(s.debts||[]).filter(d=>String(d.activity_id)===String(a.id)); const ds=activeDebtsForActivity(a.id).sort((x,y)=>pSort(player(x.player_id),player(y.player_id))); const removed=all.length-ds.length; const pending=ds.filter(d=>d.status==='pending').length; const paid=ds.filter(d=>d.status==='paid').length; const expanded=GT.expandedActivities.has(String(a.id)); const details=expanded?(ds.map(d=>debtRow(a,d)).join('')||'<div class="empty-state">Sin personas activas en esta actividad</div>')+'<div class="treasury-toolbar"><button class="btn" data-action="add-debt" data-id="'+a.id+'">Agregar persona</button></div>':''; const statusText=pending?'Pendiente':'Completada'; return '<section class="treasury-panel treasury-activity '+(expanded?'open':'collapsed')+'"><div class="treasury-panel-head treasury-activity-head"><button class="treasury-collapse-btn" type="button" data-action="toggle-activity" data-id="'+a.id+'"><span class="treasury-caret">'+(expanded?'v':'>')+'</span><span><span class="treasury-panel-title">'+h(a.name)+'</span><span class="muted">'+h(a.activity_type)+' - '+h(a.administrative_status)+' - '+paid+'/'+ds.length+' pagadas'+(removed?' - '+removed+' quitadas':'')+'</span></span></button><span class="pill '+(pending?'warn':'good')+'">'+statusText+'</span></div>'+details+'</section>'; }).join('') || legacyRows(s); document.getElementById('treasuryViewBody').innerHTML=legacy+'<div class="treasury-toolbar"><button class="btn p" data-action="new-activity">Nueva actividad/gasto</button></div>'+rows; }
  function debtRow(a,d){ const p=player(d.player_id); const amount=n(d.assigned_amount); const actions=d.status==='pending'?'<button class="btn p" data-action="pay-debt" data-id="'+d.id+'">Pagada</button><button class="btn danger" data-action="remove-debt" data-id="'+d.id+'">Quitar</button>'+wa(p,a.name,amount):''; return '<div class="treasury-card-row"><div><strong>'+h(pname(p))+'</strong><div class="muted">Beneficia a '+h(d.beneficiary_player_id?pnameById(d.beneficiary_player_id):pnameById(a.payer_player_id))+' - '+h(d.status)+'</div></div><div class="treasury-actions"><span>'+money(amount)+'</span>'+actions+'</div></div>'; }
